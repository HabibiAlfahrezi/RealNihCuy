...
<div id="alert-{{ $attributes->get('id') }}" class="fixed right-4 z-50 w-80 shadow-lg rounded-lg {{ $classes[$type] }} flex items-center">
    <div class="py-4 px-6 rounded-l-lg flex items-center">
        <!-- Ikon sesuai tipe alert -->
        @switch($type)
            @case('success')
    ...
    <div class="px-4 py-6 bg-white rounded-r-lg flex justify-between items-center w-full border border-l-transparent border-gray-200">
        <div>{{ $message }}</div>
        <button onclick="closeAlert('{{ $attributes->get('id') }}')" class="focus:outline-none">
