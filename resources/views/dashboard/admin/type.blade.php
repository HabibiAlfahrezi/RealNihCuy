...
<div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
  <div class="flex items-start justify-between">
    ...
    <div class="flex space-x-2">
      <!-- Add Button -->
      <button @click="$dispatch('open-modal', 'type')" class="border border-borderAbu rounded-lg p-1 hover:border-primary transition-colors">
              ...
              <div>
                  ...
                  <div class="grid grid-cols-4 border-b border-stroke dark:border-strokedark">
                      ...
                      <div class="flex items-center justify-center pr-3.5 py-3.5 xl:p-5">
                          <p class="font-medium text-black dark:text-white">{{ $type->pekerjaan->count() }} Jobs</p>
                      </div>

                      <div class="flex items-center justify-center pr-3.5 py-3.5 xl:p-5">
                          <span class="bg-gradient-to-tl {{ $type->status == 'active' ? 'from-green-600 to-lime-400' : ($type->status == 'close' ? 'from-red-600 to-red-400' : '') }} px-4 text-xs rounded-lg py-2 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">
                              {{ $type->status == 'active' ? 'Active' : ($type->status == 'close' ? 'Closed' : 'Inactive') }}
                          </span>
                      </div>
                      <div class="flex items-center justify-center py-3.5 xl:p-5">
                        <a href="{{ route('delete.user', $type->id) }}" data-modal-target="delete-user-modal" data-modal-toggle="delete-user-modal" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
...
<x-mainModal id="type">
  <div x-data class=" py-4">
    ...
    <form action="{{ route('admin.type') }}" id="typeForm" method="POST" class="space-y-2">
      @csrf
      ...
      <x-slot name="footer">
        ...
        <button type="submit" form="typeForm" class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 hover:bg-primary focus:outline-none focus:shadow-outline-purple">
            Add Type
        </button>
      </x-slot>
    </form>
