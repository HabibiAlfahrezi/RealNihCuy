          ...
          >
            ...
            <ul class="flex h-auto flex-col overflow-y-auto">
              @foreach (Auth::user()->notifications as $notification)
                <li>
                  <a
                    class="flex flex-col border-t border-b border-borderAbu px-4.5 py-3 hover:bg-gray-200 transition duration-300 ease-in-out dark:border-strokedark dark:hover:bg-meta-4"
                    href="#"
                  >
                    <div class="flex items-center">
                      <img src="{{ asset('storage/'. $notification->company->logo) }}" class="w-12 h-12" alt="">
                      <div class="ml-2 text-sm">
                        <h4 class="font-bold text-md">{{ $notification->company->name }}</h4>
                        <p>{{ $notification->pekerjaan->title }}</p>
                      </div>
                    </div>
                    <p class="text-sm my-2">
                      <span class="text-black dark:text-white">{{ $notification->notification }}</span>
                    </p>

                    <p class="text-xs">{{ Carbon\Carbon::parse($notification->created_at)->format('d-M-Y') }}</p>
                  </a>
                </li>
              @endforeach
            </ul>
      ...
      >
        ...
        >
          <span class="hidden text-right lg:block">
            ...
            >
            @if (Auth::user()->prosesi)
              <span class="block text-xs font-medium capitalize">{{ Auth::user()->prosesi }}</span>
            @else
              <span class="block text-xs font-medium">Pengangguran</span>
            @endif
          </span>
          ...
          <span class="h-12 w-12 rounded-full">
            @if (Auth::user()->prosesi === 'company')
              ...
              @else
                  <img src="{{ asset('img/avatar.jpg') }}" alt="Default Logo" />
              @endif
            @else
              @if (Auth::user()->profile && Auth::user()->profile->image)
                  <img src="{{ asset('storage/' . Auth::user()->profile->image) }}" alt="User" />
              @else
                  <img src="{{ asset('img/avatar.jpg') }}" alt="Default Profile Image" />
              @endif
            @endif
        ...
        >
          ...
          >
            ...
            <li>
              <a
                href="{{ route('dashboard.companysetting') }}"
                class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base"
              >
                <svg
                  class="fill-current"
