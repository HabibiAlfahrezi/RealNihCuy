            ...
            <div class="flex items-center mt-6 md:mt-0 justify-between ">
                ...
                <div class="hidden md:block">
                    <a href="{{ route('dashboard.company.create') }}"
                        class="px-5 py-3 rounded-lg bg-primary text-white">Create a Job</a>
                </div>
                <div class="block md:hidden">
                    <a href="{{ route('dashboard.company.create') }}"
                        class="px-5 py-3 rounded-lg bg-primary text-white">+</a>
                </div>
            </div>
                                    ...
                                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 min-w-max ">
                                        <thead
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-3 md:px-6 py-3 text-center whitespace-nowrap">
                                                    Status</th>
                                                <th scope="col" class="px-3 md:px-6 py-3 text-center whitespace-nowrap">
                                                    Applicant</th>
                                                <th scope="col" class="px-3 md:px-6 py-3 text-center whitespace-nowrap">
                                          ...
                                          @if ($jobs->count() > 0) 
                                            @foreach ($jobs as $job)
                                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                    <td
                                                        class="pr-3 md:px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                                        <div class="flex items-center">
                                                            <img src="{{ asset($job->company->logo ? 'storage/' . $job->company->logo : 'img/avatar.jpg') }}"
                                                                class="w-10 h-10 rounded-full mr-4"
                                                                alt="{{ $job->title }}">
                                                            <div>
                                                                <div class="text-sm font-semibold">{{ $job->title }}</div>
                                                                <div class="text-xs text-gray-500">RP.
                                                                    {{ number_format($job->salary) }} / month</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                                        @foreach ($job->type as $type)
                                                            <span
                                                                class="px-3 py-1 text-xs font-semibold text-white bg-blue-500 rounded-full">{{ $type->title }}</span>
                                                        @endforeach
                                                    </td>
                                                    <td
                                                    ...
                                                    <td class="px-3 md:px-6 py-4 text-center whitespace-nowrap">
                                                        <span class="text-xs font-semibold">{{ $job->applicant->count() }} /
                                                            ...
                                                            <div x-data="{ open: false }"
                                                                ...
                                                                <div x-show="open" @click.away="open = false"
                                                                    ...
                                                                    <div class="py-1 flex">
                                                                        ...
                                                                        <a href="{{ route('company.job.delete', $job->id) }}"
                                                                            class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
