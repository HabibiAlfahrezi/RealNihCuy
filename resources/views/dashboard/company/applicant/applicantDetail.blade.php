  ...
  <main class="container mx-auto p-6 ">
    ...
    <div class="flex flex-col lg:flex-row gap-6 mt-12">
      ...
      <div class="w-full lg:w-2/3">
        ...
        <div class="flex justify-between w-full space-x-4 mb-6 mt-3">
          <div>
          ...
          @php
            $nonInReviewApplicants = $profile->user->applicant->filter(function ($applicant) {
                return $applicant->stage != 'inreview';
            });
          @endphp
          
          @if ($nonInReviewApplicants->isNotEmpty())
            @foreach ($nonInReviewApplicants as $applicant)
                @php
                    $buttonClasses = match ($applicant->stage) {
                ...
                <div>
                  <button class="{{ $buttonClasses }} px-4 py-2 font-semibold rounded mb-2">
                    {{ ucfirst(strtolower($applicant->stage)) }}
                </button>
                </div>
            @endforeach
          @else
            <div x-data="dropdown()" class="">
              <button 
                  @click="open = !open" 
                  class=" text-gray-700 font-semibold border border-borderAbu px-4 py-2 flex items-center transition duration-300 ease-in-out "
              >
        ...
        <div class="space-y-8 pt-7">
          <div class="grid grid-cols-2 gap-6 pb-6 border-b border-borderAbu">
            <!-- Email -->
            ...
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                @if ($profile->birth_date == null)
                  <p class="text-gray-500 text-sm">No Birth Date</p>
                @else   
                  <p class="text-gray-500 text-sm">{{ \Carbon\Carbon::parse($profile->birth_date)->age ?? 'No Age' }} Years Old</p>
                @endif
              </div>
            </div>
        ...
        <div class="flex justify-between items-center border-b border-borderAbu pb-4 mb-4">
          ...
          <a href="{{ asset('storage/' . ($profile->userDocument->first()->document ?? '')) }}" 
           ...
           <div class="rounded-full p-1 md:mr-2">
        ...
        <div class="space-y-7">
          @foreach ($profile->userEducation as $education)
            <div class="flex items-start  border-b border-borderAbu pb-7">
              <img src="{{ asset('storage/'. $education->image)  }}" alt="University Logo" class="w-14 h-14 rounded-full " />
              <div class="flex-grow">
                <div class="flex justify-between items-center">
      ...
      <div class="w-full lg:w-1/3 md:px-10">
        ...
        <form action="{{ route('role.store') }}" method="POST">
          @csrf
          <select disabled name="role" onchange="this.form.submit()" class="border border-borderAbu px-4 py-2 rounded text-gray-700 w-full">
      ...
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
        ...
        <div class="flex-auto ">
          <div class="flex flex-wrap gap-6">
            ...
            <div class="w-full max-w-full mb-6 md:w-6/12 md:flex-none xl:mb-8 xl:w-3/12 border border-borderAbu shadow-lg py-4 px-6 rounded-lg bg-white"> {{-- Tambahkan background untuk memastikan border terlihat jelas --}}
  ...
  <div x-data class="py-4 max-w-xl mx-auto bg-white ">
    ...
    <div>
      <form action="{{ route('education.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex justify-end space-x-2">
          <button type="button" @click="$dispatch('close-modal')" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">Cancel</button>
          <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">Add Education</button>
        </div>
      </form>
    </div>
  </div>
